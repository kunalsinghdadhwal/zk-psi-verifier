...
fn test_serialization_deserialization() {
    let k = 10;
    let (params, pk, vk) = setup(k).unwrap();
    ...
    // Deserialize
    let pk_recovered: halo2_proofs::plonk::ProvingKey<pasta_curves::EqAffine> = 
        bincode::deserialize(&pk_bytes).unwrap();
    let vk_recovered: halo2_proofs::plonk::VerifyingKey<pasta_curves::EqAffine> = 
